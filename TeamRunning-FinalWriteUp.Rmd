---
title: "Simulating National Cross Country Meets"
...
Sys.setenv("plotly_username"="ecaa2017")
Sys.setenv("plotly_api_key"="YixbeTuCDflejGIr8e9k")
...
```{r Initial Codestuff, cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
###Initial codestuff
...
```{r Get Team Names, cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
###########
...
```{r URLS, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r scraper, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
team_races<-list()
for(url in urls) {
  print(url)
  webpage <- read_html(url)
...
```{r Bijans Race Adjustments, cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Lubridate, cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r SIMULATE, cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
set.seed(11)
sim_stats<-list()
total_stats<-list()
...
```{r Interactive Plot with Win Probabilities, cache=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
#######
...
```{r 2018, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r Course Corrections, cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Simulate, cache=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
set.seed(11)
sim_stats<-list()
total_stats<-list()
...
```{r Interactive Plot with Win Probabilities 2018, cache=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
b<-sim_stats %>% group_by(TEAMS) %>% summarize(On_Podium=n()/100, P_first=sum(PLACES==1)/100, P_second=sum(PLACES==2)/100, P_third=sum(PLACES==3)/100, P_fourth=sum(PLACES==4)/100) %>% dplyr::arrange(desc(On_Podium))
...
We thought, hey, maybe teams which travel further for nationals have harder time performing to expectation at the national meet. We use this map to illustrate the travel distances for each team. One thing of note is that most teams in the Midwest drive to nationals, almost regardless of where the meet is. Air travel is stressful for recovery which could affect runners' perfomance in the national meet.
```{r Map, cache=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
library(maps)

dfPoints=data.frame(lat=c(41.772769, 38.648823, 43.817551, 40.009308, 42.728922, 42.370855, 34.101537, 42.712813, 39.329901, 42.796139, 41.788720, 42.729700, 44.461325, 44.008000, 42.360162, 40.442748, 42.929477, 41.504277, 44.871635, 44.461663, 37.572379, 44.105683, 34.101217, 40.125502, 42.471333, 44.943629, 33.797065, 44.026202, 38.212276), 
                    long=c(-88.143896, -90.310903, -91.231014, -75.307651, -92.482281, -72.517003, -117.713866, -73.203150, -76.620507, -77.823939, -87.598681, -73.678770, -93.156124, -73.176009, -71.094117, -79.943116, -85.588804, -81.608395, -91.926698, -93.182732, -84.290675, -70.202229, -117.714030, -82.936466, -75.062472, -93.190534, -84.322369, -88.550783, -85.758406),
                    places=paste(c(intersect(results2018$TEAM, filtered_results$TEAM), "Oshkosh", "Louisville")))
maps::map("usa")
points(x=dfPoints$long, y=dfPoints$lat, col = "blue", pch=20, cex=1)
points(x = dfPoints %>% filter(places!= "Oshkosh") %>% filter(places!= "Louisville") %>% select(long) %>% unlist(), y = dfPoints %>% filter(places!= "Oshkosh") %>% filter(places!= "Louisville") %>% select(lat) %>% unlist(), col = "red", pch=20, cex=1)
text("Louisville", x=tail(dfPoints$long, 1), y=tail(dfPoints$lat, 1),  col="slateblue", cex=0.5, pos=3)
text("Oshkosh", x=dfPoints$long[28], y=dfPoints$lat[28], col="slateblue", cex=0.5, pos=3)
text("North Central", x=dfPoints$long[1], y=dfPoints$lat[1], col="slateblue", cex=0.5, pos=3)
text("Wash U", x=dfPoints$long[2], y=dfPoints$lat[2], col="slateblue", cex=0.5, pos=3)
text("Claremont", x=dfPoints$long[7], y=dfPoints$lat[7], col="slateblue", cex=0.5, pos=3)
text("Williams", x=dfPoints$long[8], y=dfPoints$lat[8], col="slateblue", cex=0.5, pos=3)
```
#Conclusion
We found a few compelling results with this project. 1) We saw that it was a reasonable assumption to model runners' performances as coming from a normal distribution. 2) We saw that race adjustments need not be as computationally intensive as using least-squares to fit an adjustment time to a matrix of pairwise adjustments. And 3) While our simulations did a pretty good job of predicting podium finishes, making predictions for races has a lot of uncertainty, as evidenced by the range of finishing places over 100 simulations. The most challenging and time-consuming part of this project was building the scraper to reliably parse results from TFRRS.
