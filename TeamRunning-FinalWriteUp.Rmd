...
#########################
all_results<-read.csv("2019_all_results.csv")
...
View(split_results)
for(person in split_results){
  random_shit <- person %>% group_by(NAME) %>% summarize(MEAN=mean(ADJ_TIME, na.rm=TRUE), SD=sd(ADJ_TIME, na.rm = TRUE))
}
